...
import { ChevronRight, ChevronLeft, Star, Search, Monitor, CheckCircle, AlertCircle, BookOpen, Trophy, Lightbulb, Target, Eye, Code2, Zap, HelpCircle, ArrowDown, ArrowRight, X } from 'lucide-react';
...
export const TutorialVanishingBloggerCase: React.FC<TutorialVanishingBloggerCaseProps> = ({ onComplete, onBack }) => {
  ...
  const validateCurrentStep = () => {
    if (!currentStep.validationFunction) return true;
    return currentStep.validationFunction(htmlCode, cssCode);
  };
      ...
      {showCompletion && (
        <CaseCompletion
          ...
          onHome={async () => {
            ...
            try {
              // Use completeCase to handle tutorial completion with anti-grinding logic
              const result = await completeCase('case-vanishing-blogger', score, Date.now());
              
              if (result.isRepeat) {
                // Show friendly message for repeat completion
                setToastMessage(`Tutorial completed again! No points awarded. Try other cases to earn more points!`);
                setToastType('info');
                setShowToast(true);
              } else {
                // Show success message for first completion
                setToastMessage(`Tutorial completed! You earned ${result.pointsAwarded} points!`);
                setToastType('success');
                setShowToast(true);
              }
              
              // Still call the original onComplete for navigation
