...
export const DetectiveCaseInterface: React.FC<DetectiveCaseInterfaceProps> = ({
  ...
  useEffect(() => {
    if (gamePhase === 'mission' && currentMission && shouldResetCode) {
      console.log('🔄 RESETTING CODE FOR MISSION:', {
        ...
        firstHtmlLine: currentMission.brokenHtml.split('\n')[0],
        firstCssLine: currentMission.brokenCss.split('\n')[0]
      });
      ...
      console.log('✅ Code reset completed for mission:', currentMission.id);
    }
    ...
    if (gamePhase === 'mission' && currentMission && currentHtml && currentCss) {
      ...
      const timeoutId = setTimeout(() => {
        ...
        if (isCodeInCompleteState(currentHtml, currentCss)) {
          ...
          // Debug logging
          console.log('🎮 Mission validation (debounced):', {
            missionId: currentMission.id,
            missionTitle: currentMission.title,
            missionIndex: currentMissionIndex + 1,
          ...
          if (currentMission.id === 'clue-2') {
            console.log('🔍 MISSION 2 DEBUGGING:');
            console.log('  HTML contains #insta-clue:', currentHtml.toLowerCase().includes('id="insta-clue"'));
            console.log('  CSS contains #insta-clue:', currentCss.toLowerCase().includes('#insta-clue'));
            console.log('  CSS contains display: none:', currentCss.toLowerCase().includes('display: none'));
            console.log('  CSS contains display: block:', currentCss.toLowerCase().includes('display: block'));
            console.log('  Full CSS for #insta-clue:', currentCss.match(/#insta-clue[^}]*}/gi));
          }
          
          if (validation.isCompleted && !missionCompleted) {
            console.log('Mission completed! Revealing clue:', currentMission.clueRevealed);
  ...
  const handleNextMission = () => {
    console.log('🚀 MISSION TRANSITION:', {
    ...
    if (currentMissionIndex < caseData.missions.length - 1) {
      setCurrentMissionIndex(prev => {
        ...
        console.log('🎯 Setting new mission index:', newIndex);
        return newIndex;
      });
      setMissionCompleted(false);
      ...
      console.log('🏁 All missions completed, showing completion');
