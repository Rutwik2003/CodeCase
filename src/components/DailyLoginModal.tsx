...
export const DailyLoginModal: React.FC<DailyLoginModalProps> = ({ isOpen, onClose }) => {
  ...
  const claimDailyReward = async () => {
    ...
    try {
      ...
      console.log('Daily Login Reward Update:', {
          ...
          <div className="p-6 border-b border-slate-700 bg-gradient-to-r from-amber-500/10 to-orange-500/10">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-100">Daily Investigation</h2>
                  <p className="text-slate-400">Claim your daily detective rewards</p>
                </div>
              </div>
              <button
                onClick={onClose}
          ...
          <div className="p-6 border-b border-slate-700">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <p className="text-3xl font-bold text-slate-100">{currentStreak} Days</p>
              ...
              <div className="flex items-center space-x-4">
                {canClaimToday && !todaysClaimed && (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="flex items-center space-x-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-6 py-3 rounded-xl font-medium hover:from-green-600 hover:to-emerald-700 transition-all disabled:opacity-50"
                  >
                ...
                {todaysClaimed && nextClaimTime && (
                  <div className="space-y-3">
                    <div className="flex items-center space-x-2 text-green-400 bg-green-500/20 px-4 py-2 rounded-xl">
                      <CheckCircle className="w-5 h-5" />
                      <span>Reward claimed!</span>
                    </div>
                    <div className="text-center text-slate-400 bg-slate-800/50 px-4 py-3 rounded-xl">
                      <p className="text-sm mb-1">⏰ Next claim available in:</p>
                      <p className="text-lg font-mono text-amber-400">{timeUntilNextClaim}</p>
                    </div>
                  </div>
                )}
                ...
                {!canClaimToday && !todaysClaimed && (
                  <div className="text-center text-slate-400 bg-slate-800/50 px-4 py-3 rounded-xl">
                    <p className="text-sm">⏰ Come back when your 24-hour cooldown is over!</p>
                  </div>
                )}
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    {!isCompleted && !isCurrent && !isToday && (
                      <div className="absolute -top-2 -right-2">
                        <Lock className="w-6 h-6 text-slate-600 bg-slate-900 rounded-full" />
                      </div>
                    )}
                    ...
                    {isCurrent && (
                      <div className="absolute -top-2 -right-2">
                        ...
                        >
                          <Gift className="w-6 h-6 text-amber-400 bg-slate-900 rounded-full" />
                        </motion.div>
                      </div>
                    )}
                  </motion.div>
            ...
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2 text-slate-400">
                  ...
                  <span>Login daily to maintain your streak</span>
                </div>
              </div>
